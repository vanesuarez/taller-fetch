...
fetch(DATA_URL)
  .then(response => response.json()) // Se asume que la respuesta es notación JSON y devuelve el objeto construido a partir de dicha notación (convierte la respuesta en formato JSON)
  .then(data => {
    showData(data.students); // Utiliza la funcion showData para mostrar los datos de los estudiantes
  })
  .catch(error => { // Si hay algun error, imprimimos el siguiente mensaje en la consola
